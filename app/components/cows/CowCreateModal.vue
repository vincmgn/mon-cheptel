...
async function onSubmit() {
  ...
  try {
    await $fetch('/api/v1/cows', {
      method: 'POST',
      body: { officialId: officialId.value.trim(), penId: props.penId, prophylaxis: prophylaxis.value },
    })
      ...
      <template #body>
        <div class="space-y-4">
          <UFormField label="Numéro officiel" required>
            <UInput v-model="officialId" placeholder="Ex: FR 12345678901" autofocus class="w-full" />
          </UFormField>
          <UFormField>
          ...
          <div class="flex justify-end gap-2 pt-2">
            ...
            <UButton :loading="isCreating" :disabled="!officialId.trim()" @click="onSubmit">Ajouter</UButton>
          </div>
        </div>
      </template>
    </UModal>
