...
async function addBreeding() {
  isAdding.value = true
  try {
    const body: Record<string, unknown> = { cowId: props.cow.id, date: date.value, isMaybe: isMaybe.value }
    if (bullType.value === 'existing' && bullId.value) body.bullId = bullId.value
    else if (bullType.value === 'external' && bullName.value.trim()) body.bullName = bullName.value.trim()
    ...
    <UModal v-model:open="isOpen" title="Enregistrer une saillie">
      <template #body>
        <div class="space-y-4">
          ...
          <UFormField>
            <UCheckbox v-model="isMaybe" label="Résultat incertain (peut-être ?)" />
          </UFormField>
