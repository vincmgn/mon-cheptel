  ...
  <div>
    <div class="mt-4 flex-1">
      ...
      <ul v-if="props.location.buildings.length" class="space-y-1.5">
        <li v-for="building in props.location.buildings" :key="building.id" class="flex items-center justify-between text-sm">
          <span class="flex items-center gap-1.5 text-gray-700 dark:text-gray-300">
            <UIcon name="i-lucide-building-2" class="size-4 text-gray-400 shrink-0" />
            {{ building.name }}
          </span>
