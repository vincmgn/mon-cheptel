...
<template>
  <div>
    <UModal :open="open" title="Supprimer le bâtiment" @update:open="emit('close')">
      <template #body>
        <p class="text-gray-700 dark:text-gray-300">
          Êtes-vous sûr de vouloir supprimer <strong>{{ building?.name }}</strong> ?
        </p>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">
          Cette action est irréversible. La suppression échouera si des cases sont encore associées.
        </p>
