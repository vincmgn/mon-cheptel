...
async function onSubmit() {
  ...
  try {
    await $fetch('/api/v1/pens', {
      method: 'POST',
      body: { name: name.value.trim(), buildingId: props.buildingId },
    })
...
<template>
  <div>
    <UModal :open="open" title="Nouvelle case" @update:open="emit('close')">
      <template #body>
        <div class="space-y-4">
          <UFormField label="Nom" required>
            <UInput v-model="name" placeholder="Ex: Box 1" autofocus class="w-full" />
          </UFormField>
