  ...
  <UContainer class="py-10 max-w-5xl">
    <div v-if="building">
      <div class="flex items-end justify-between">
        ...
        <div class="mb-8">
          <UButton icon="i-lucide-plus" @click="isCreateOpen = true">Nouvelle case</UButton>
        </div>
      </div>
      ...
      <!-- Empty state -->
      <PenEmptyState v-else-if="!building.pens.length" @click="isCreateOpen = true" />
        ...
        <UCard v-for="pen in building.pens" :key="pen.id" class="flex flex-col">
          <div class="flex items-start justify-between">
            ...
            <div class="flex gap-1 ml-2 shrink-0">
              <UButton icon="i-lucide-pencil" color="neutral" variant="subtle" size="md" aria-label="Modifier" @click="openEdit(pen)" />
              <UButton icon="i-lucide-trash-2" color="error" variant="subtle" size="md" aria-label="Supprimer" @click="openDelete(pen)" />
            </div>
          </div>
          <PenCard :pen="pen" />
        </UCard>
    ...
    <PenCreateModal :open="isCreateOpen" :building-id="id" @created="refresh" @close="isCreateOpen = false" />
    <PenEditModal :open="isEditOpen" :pen="editTarget" @updated="refresh" @close="isEditOpen = false" />
    <PenDeleteModal :open="isDeleteOpen" :pen="deleteTarget" @deleted="refresh" @close="isDeleteOpen = false" />
  </UContainer>
</template>
