...
<template>
  <UContainer class="py-10 max-w-5xl">
    ...
    <div v-if="status === 'pending'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div v-for="i in 3" :key="i" class="h-40 rounded-xl animate-pulse bg-gray-100 dark:bg-gray-800" />
    </div>

    <!-- Empty state -->
    <LocationEmptyState v-else-if="locations.length === 0" @click="isCreateOpen = true" />
      ...
      <UCard v-for="location in locations" :key="location.id" class="flex flex-col">
        ...
        <div class="flex items-start justify-between">
          <div class="flex items-center gap-2 min-w-0">
            <div class="p-2 rounded-lg bg-primary/10 dark:bg-primary/20 shrink-0 flex items-center justify-center">
          ...
          <div class="flex gap-1 ml-2 shrink-0">
            <UButton icon="i-lucide-pencil" color="neutral" variant="subtle" size="md" aria-label="Modifier"
    ...
    <!-- Delete Modal -->
    <LocationDeleteModal :open="isDeleteOpen" :location="deleteTarget" @deleted="refresh"
