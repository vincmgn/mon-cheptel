  ...
  <UContainer class="py-10 max-w-5xl">
    <div v-if="location">
      <div class="flex items-end justify-between mb-0">
        ...
        <div class="mb-8">
          <UButton icon="i-lucide-plus" @click="isCreateOpen = true">Nouveau bâtiment</UButton>
        </div>
      </div>
      ...
      <!-- Empty state -->
      <BuildingEmptyState v-else-if="!location.buildings.length" @click="isCreateOpen = true" />
        ...
        <UCard v-for="building in location.buildings" :key="building.id" class="flex flex-col">
          <div class="flex items-start justify-between">
            ...
            <div class="flex gap-1 ml-2 shrink-0">
              <UButton icon="i-lucide-pencil" color="neutral" variant="subtle" size="md" aria-label="Modifier" @click="openEdit(building)" />
              <UButton icon="i-lucide-trash-2" color="error" variant="subtle" size="md" aria-label="Supprimer" @click="openDelete(building)" />
            </div>
          </div>
    ...
    <BuildingCreateModal :open="isCreateOpen" :location-id="id" @created="refresh" @close="isCreateOpen = false" />
    <BuildingEditModal :open="isEditOpen" :building="editTarget" @updated="refresh" @close="isEditOpen = false" />
    <BuildingDeleteModal :open="isDeleteOpen" :building="deleteTarget" @deleted="refresh" @close="isDeleteOpen = false" />
  </UContainer>
</template>
