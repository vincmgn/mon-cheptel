...
interface BackupBull { name: string; createdAt?: string }
interface BackupCow { officialId: string; location: string; building: string; pen: string; prophylaxis?: boolean; createdAt?: string }
interface BackupBreeding { date: string; cowOfficialId: string; bullName?: string | null; isMaybe?: boolean }
interface BackupCalf { officialId?: string | null; sex: string; birthDate: string; motherOfficialId: string }
...
export default defineEventHandler(async event => {
  ...
  if (backup.bulls?.length) {
    const existing = await prisma.bull.findMany({ where: { userId }, select: { name: true } })
    const existingNames = new Set(existing.map(b => b.name.toLowerCase()))

    for (const row of backup.bulls) {
      if (!row.name?.trim()) { result.bulls.skipped++; continue }
      if (existingNames.has(row.name.trim().toLowerCase())) { result.bulls.skipped++; continue }
      const createdAt = parseDate(row.createdAt)
      try {
        await prisma.bull.create({
          data: { name: row.name.trim(), userId, ...(createdAt ? { createdAt } : {}) },
        })
        existingNames.add(row.name.trim().toLowerCase())
        result.bulls.created++
      } catch { result.bulls.skipped++ }
    }
  }
  ...
  if (backup.cows?.length) {
    ...
    for (const row of backup.cows) {
      if (!row.officialId?.trim()) { result.cows.skipped++; continue }
      if (existingIds.has(row.officialId.trim())) { result.cows.skipped++; continue }
      ...
      if (!pen) { result.cows.skipped++; continue }
  ...
  if (backup.breedings?.length) {
    ...
    const bulls = await prisma.bull.findMany({ where: { userId }, select: { id: true, name: true } })
    ...
    const existingKeys = new Set(existing.map(b => `${b.cowId}:${isoDay(b.date)}`))

    for (const row of backup.breedings) {
      if (!row.date?.trim() || !row.cowOfficialId?.trim()) { result.breedings.skipped++; continue }
      const parsedDate = parseDate(row.date)
      if (!parsedDate) { result.breedings.skipped++; continue }
      const cowId = cowMap.get(row.cowOfficialId.trim())
      if (!cowId) { result.breedings.skipped++; continue }
      const key = `${cowId}:${isoDay(parsedDate)}`
      if (existingKeys.has(key)) { result.breedings.skipped++; continue }

      const bullId = row.bullName?.trim() ? bullMap.get(row.bullName.trim().toLowerCase()) ?? null : null
      const bullName = !bullId && row.bullName?.trim() ? row.bullName.trim() : null
      ...
      } catch { result.breedings.skipped++ }
    }
  }
  ...
  if (backup.calves?.length) {
    ...
    const existing = await prisma.calf.findMany({
      where: { officialId: { not: null }, cow: { pen: { building: { location: { userId } } } } },
      select: { officialId: true },
    })
    ...
    for (const row of backup.calves) {
      if (!row.sex?.trim() || !row.birthDate?.trim() || !row.motherOfficialId?.trim()) {
        result.calves.skipped++; continue
      }
      const birthDate = parseDate(row.birthDate)
      if (!birthDate) { result.calves.skipped++; continue }
      const cowId = cowMap.get(row.motherOfficialId.trim())
      if (!cowId) { result.calves.skipped++; continue }
      ...
      if (officialId && existingIds.has(officialId.toLowerCase())) { result.calves.skipped++; continue }

      try {
        await prisma.calf.create({
          data: { cowId, sex, birthDate, ...(officialId ? { officialId } : {}) },
        })
        if (officialId) existingIds.add(officialId.toLowerCase())
        result.calves.created++
      } catch { result.calves.skipped++ }
    }
  }
